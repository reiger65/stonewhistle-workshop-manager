...
export function extractSpecifications(lineItem: ShopifyOrderResponse['line_items'][0], serialNumber?: string): Record<string, string> {
  ...
  if (lineItem.title) {
    ...
    const noteMatch = lineItem.title.match(/[A-G](#|b)?m?\d/);
    if (noteMatch) {
      specs['tuning'] = noteMatch[0];
    }
  }
    ...
    for (let i = 0; i < shopifyOrders.length; i++) {
      ...
      for (let i = 0; i < allLineItems.length; i++) {
        ...
        for (let q = 0; q < quantity; q++) {
          ...
          // Roep de functie aan die al geïmporteerd is bovenaan het bestand
          ...
          try {
            ...
            const item = await storage.findOrCreateOrderItem({
              ...
              archivedReason: !isItemFulfillable ? 'Automatisch gearchiveerd omdat het item al is fulfilled in Shopify' : null
            }, {
