...
export default function Worksheet() {
  ...
  // Fetch only active resellers, GEEN periodieke verversing meer nodig (alleen bij eerste laden en expliciete invalidations)
  const { data: resellers = [] } = useQuery<Reseller[]>({
    queryKey: ['/api/resellers/active'],
    // Schakel alle vormen van automatische refetching uit
    refetchInterval: false,  // Geen automatische verversing meer
    staleTime: Infinity,     // Data wordt nooit automatisch als verouderd beschouwd
  ...
  const itemsByOrder = useMemo(() => {
    ...
    return enrichedItems.reduce((acc, item) => {
      ...
      // STAP 6: EXTRA DEDUPLICATIE - Controleer of dit item al bestaat in de groep (op basis van id of serialNumber)
      const itemAlreadyExists = acc[orderId].some(existingItem => {
        // Controleer eerst op id (meest betrouwbaar)
        if (existingItem.id === normalizedItem.id) {
          console.log(`[DEDUPLICATIE] Item met id ${existingItem.id} bestaat al voor order ${orderId}`);
          return true;
        }
        
        // Controleer vervolgens op serialNumber (als backup)
        if (normalizedItem.serialNumber && 
            existingItem.serialNumber === normalizedItem.serialNumber) {
          console.log(`[DEDUPLICATIE] Item met serialNumber ${existingItem.serialNumber} bestaat al voor order ${orderId}`);
          return true;
        }
  ...
  const validOrderIds = useMemo(() => {
    console.log(`VALID ORDERS: Rebuilding validOrderIds based on ${filteredOrderItems.length} filtered items`);
...
const SERIAL_NUMBER_DATABASE: Record<string, {
  ...
  'SW-1542-1': {type: 'INNATO', tuning: 'Dm4', frequency: '432', color: 'SB'},
  'SW-1542-2': {type: 'INNATO', tuning: 'A3', frequency: '432', color: 'SB'},
  'SW-1542-3': {type: 'INNATO', tuning: 'Em4', frequency: '432', color: 'SB'},
  'SW-1542-4': {type: 'INNATO', tuning: 'C4', frequency: '432', color: 'SB'},
  'SW-1542-5': {type: 'NATEY', tuning: 'Dm4', frequency: '432', color: 'T'},
  'SW-1542-6': {type: 'INNATO', tuning: 'Cm4', frequency: '432', color: 'SB'},
  'SW-1542-7': {type: 'INNATO', tuning: 'Bm3', frequency: '432', color: 'SB'},
  'SW-1542-8': {type: 'INNATO', tuning: 'Bm3', frequency: '432', color: 'SB'},
  'SW-1542-9': {type: 'INNATO', tuning: 'Am3', frequency: '432', color: 'SB'},
  'SW-1542-10': {type: 'NATEY', tuning: 'A3', frequency: '432', color: 'T'},
  'SW-1542-11': {type: 'INNATO', tuning: 'Gm3', frequency: '432', color: 'SB'},
  'SW-1542-12': {type: 'INNATO', tuning: 'Gm3', frequency: '432', color: 'SB'},
  'SW-1542-13': {type: 'INNATO', tuning: 'G#m3', frequency: '432', color: 'SB'},
  'SW-1542-14': {type: 'NATEY', tuning: 'Am3', frequency: '432', color: 'T'},
  'SW-1542-15': {type: 'NATEY', tuning: 'Am3', frequency: '432', color: 'T'},
  
  // Enkele nummers weglaten om database niet te groot te maken
  'SW-1542-48': {type: 'INNATO', tuning: 'Em4', frequency: '432', color: 'SB'},
  'SW-1542-49': {type: 'INNATO', tuning: 'G3', frequency: '432', color: 'SB'},
  'SW-1542-53': {type: 'INNATO', tuning: 'Em4', frequency: '432', color: 'SB'},
  'SW-1542-54': {type: 'INNATO', tuning: 'Cm4', frequency: '432', color: 'SB'},
  'SW-1542-57': {type: 'NATEY', tuning: 'F3', frequency: '432', color: 'B'},
  'SW-1542-59': {type: 'INNATO', tuning: 'D4', frequency: '432', color: 'SB'},
  'SW-1542-60': {type: 'INNATO', tuning: 'Gm3', frequency: '432', color: 'SB'},
  'SW-1542-61': {type: 'NATEY', tuning: 'C4', frequency: '432', color: 'T'},
  'SW-1542-63': {type: 'INNATO', tuning: 'Gm3', frequency: '432', color: 'SB'},
  'SW-1542-65': {type: 'INNATO', tuning: 'Em4', frequency: '432', color: 'SB'},
