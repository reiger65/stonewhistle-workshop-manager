...
export default function Worksheet() {
  ...
  const calculateAverageWaitTime = useMemo(() => {
    ...
    // Look at orders that have been delivered within the last year
    const oneYearAgo = new Date();
    oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
    
    // Consider only completed orders with valid dates for the calculation
    const completedOrders = (allOrders as Order[]).filter(order => 
      (order.status === 'delivered' || order.status === 'shipping') && 
      order.orderDate && 
      (order.shippedDate || order.deliveredDate) &&
      // Only include orders that were completed within the last year
      ((order.shippedDate && new Date(order.shippedDate) > oneYearAgo) ||
       (order.deliveredDate && new Date(order.deliveredDate) > oneYearAgo))
    );
    ...
    for (const order of completedOrders) {
      ...
      const completionDate = order.shippedDate ? new Date(order.shippedDate) : 
                            order.deliveredDate ? new Date(order.deliveredDate) : null;
      
      if (!completionDate) continue; // Skip if no completion date
      
      // Calculate wait time in days
      const waitTimeDays = Math.round((completionDate.getTime() - orderDate.getTime()) / (1000 * 60 * 60 * 24));
      
      // Skip negative wait times or extremely long wait times (data issues)
      ...
      totalDays += cappedWaitTimeDays;
      count++;
    }
    
    return count > 0 ? Math.round(totalDays / count) : 45; // Default to 45 days if no data
  }, [allOrders]);
      ...
      <div className="flex flex-wrap gap-2 items-center mb-4 bg-gray-50 p-3 rounded-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-8 bg-blue-100 hover:bg-blue-200 text-blue-800 border-blue-300"
          >
            <BarChart className="h-4 w-4 mr-1" />
            Orders: {unfulfilledOrdersCount}
          </Button>
          ...
          <Button
            variant="outline"
            size="sm" 
            className="h-8 bg-amber-100 hover:bg-amber-200 text-amber-800 border-amber-300"
          >
            <TimerIcon className="h-4 w-4 mr-1" />
            Wait: ~{calculateAverageWaitTime} days
          </Button>
        </div>
      </div>
    );
