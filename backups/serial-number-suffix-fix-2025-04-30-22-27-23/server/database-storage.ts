...
import { db } from "./db";
import { eq, and, desc, asc, or, like, ilike, isNull, gte, lte, sql } from "drizzle-orm";
    ...
    return await db
      ...
      .where(
        and(
          ...
          eq(orderItems.isArchived, false) // Alleen niet-gearchiveerde items ophalen
        )
      )
      .orderBy(asc(orderItems.serialNumber));
  }

  // Vind ook gearchiveerde items (voor admin doeleinden)
  async getAllOrderItems(orderId: number, includeArchived: boolean = false): Promise<OrderItem[]> {
