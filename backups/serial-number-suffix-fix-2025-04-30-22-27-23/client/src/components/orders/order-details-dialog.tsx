...
import { FileText, X, ExternalLink, CheckCircle2, TruckIcon, MessageCircle } from 'lucide-react';
import { formatDate, getStatusIcon, getStatusLabel } from '@/lib/utils';
import { Order, OrderItem, ProductionNote } from '@shared/schema';
...
export function OrderDetailsDialog({ open, onClose, order, orderItems }: OrderDetailsDialogProps) {
  ...
  // Opmerking query om alle opmerkingen op te halen
  const { data: productionNotes = [] } = useQuery<ProductionNote[]>({
    queryKey: [`/api/orders/${order.id}/notes`],
    enabled: open && !!order.id,
  });
  
  // Reset notes wanneer order verandert
  useEffect(() => {
  ...
  // Functie om een nieuwe productie opmerking toe te voegen
  const addNoteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/orders/${order.id}/notes`] });
      setNewNote('');
      toast({
        title: "Note Added",
        description: "Production note has been added successfully",
      });
    },
    onError: () => {
      toast({
        ...
        description: "Could not add production note",
      });
    }
  });
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="overflow-y-auto p-6">
          ...
          <div>
            ...
            {/* Lijst met bestaande opmerkingen */}
            {productionNotes && productionNotes.length > 0 && (
              <div className="mb-4 space-y-3">
                {productionNotes.map((note) => (
                  <div key={note.id} className="border dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-800">
                    <div className="flex items-start">
                      {/* Icoon voor opmerking bron: intern (blauw) of Shopify (groen) */}
                      {note.source === 'shopify' ? (
                        <div className="shopify-note-icon" style={{color: 'green', borderTop: '2px solid green'}}>
                          <SiShopify className="h-4 w-4 mt-1 text-green-500 mr-2 flex-shrink-0" style={{color: 'green'}} />
                        </div>
                      ) : (
                        <div className="internal-note-icon" style={{color: 'blue', borderTop: '2px solid blue'}}>
                          <MessageCircle className="h-4 w-4 mt-1 text-[#0000FF] mr-2 flex-shrink-0" style={{color: 'blue'}} />
                        </div>
                      )}
                      ...
                      <div className="flex-grow">
                        ...
                        <div className="mt-1 flex justify-between items-center text-xs text-gray-500">
                          <div>{note.createdBy} • {formatDate(note.createdAt)}</div>
                          <div className="text-xs text-gray-400 italic font-bold">
                            {note.source === 'shopify' ? '*** Via Shopify ***' : '*** Interne notitie ***'}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {/* Toevoegen van een nieuwe opmerking */}
            <div className="border dark:border-gray-700 rounded-lg overflow-hidden">
              <OptimizedTextarea 
                value={newNote} 
                onChange={setNewNote} 
                ...
                placeholder="Voeg een nieuwe productie notitie toe..."
              />
              <div className="bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-700 p-3 flex justify-between items-center">
                <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center">
                  <MessageCircle className="h-3 w-3 text-[#0000FF] mr-1" style={{color: 'blue'}} />
            ...
            {/* Bestaande oude notities (legacy) */}
            {notes && (
              <div className="mt-4 border dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900">
                <div className="font-medium text-sm mb-1">Oude notities:</div>
