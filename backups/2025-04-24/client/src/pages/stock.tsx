...
function MaterialDialog({ isOpen, onOpenChange, material, onSave }) {
  ...
  async function onSubmit(data) {
    try {
      ...
      if (material?.id) {
        await apiRequest("PATCH", `/api/materials/${material.id}`, cleanData);
        toast({
          title: "Material updated",
          description: "The material has been successfully updated.",
        });
      } else {
        await apiRequest("POST", "/api/materials", cleanData);
        toast({
  ...
  return (
    <MainLayout>
      <div className="container mx-auto py-6 space-y-6">
        ...
        {materialsQuery.data?.some(material => material.quantity <= material.reorderPoint) && (
