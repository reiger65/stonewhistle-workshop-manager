...
export default function Completed() {
  ...
  const unfulfilledOrdersCount = (allOrders as Order[]).filter(order => 
  ...
  const completedOrders = (allOrders as Order[]).filter((order: Order) => {
    // Include orders marked as shipping or delivered
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center justify-center px-2 py-0.5 text-xs font-semibold bg-[#C26E50] text-white rounded">
            {unfulfilledOrdersCount} in progress
          </span>
        </div>
