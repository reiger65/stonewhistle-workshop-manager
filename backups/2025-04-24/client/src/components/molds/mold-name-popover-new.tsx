...
export function MoldNamePopover({ children, instrumentType, tuningNote, frequency }: MoldNamePopoverProps) {
  ...
  const instrumentHasMolds = (type: string): boolean => {
    const lowerType = type.toLowerCase();
    // Natey, Double, and Innato flutes have molds
  ...
  const getMoldErrorMessage = (type: string): string => {
    ...
    if (lowerType.includes('natey')) {
      ...
      return "ZEN flutes don't use molds";
  ...
  useEffect(() => {
    if (open && instrumentType && tuningNote) {
      ...
      if (!instrumentHasMolds(safeInstrumentType)) {
        // Skip fetching for instruments that don't have molds (like ZEN)
        setMolds([]);
        setIsLoading(false);
        return;
      }
      ...
      console.log(`Making API request with encoded tuning note: ${encodedTuningNote} (original: ${safeTuningNote})`);
      
      fetch(`/api/instrument-molds/${safeInstrumentType}/${encodedTuningNote}`)
        .then(res => {
          if (!res.ok) {
            if (res.status === 404) {
                ...
                <section>
                  ...
                  <div className="w-full">
                    <TunerSettingsProvider>
                      <VesselBasedTuner 
                        fluteType={safeInstrumentType}
                        tuningKey={safeTuningNote} 
                        frequency={safeFrequency || "440"}
                        moldName={molds && molds[0] ? molds[0].name : undefined}
                      />
                    </TunerSettingsProvider>
                  </div>
                </section>
