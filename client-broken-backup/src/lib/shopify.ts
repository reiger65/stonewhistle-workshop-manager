 ...
 * @param {string} [period] - Optional period to limit the sync ("1week", "1month", "3months", "6months", "1year", "all")
 */
export async function syncShopifyOrders(period?: string): Promise<{ 
  message: string;
  ...
  try {
    const response = await apiRequest('POST', '/api/import-shopify', { period });
