// NEW FILE 2025-01-27 - BYPASS CACHE - resellers2.map fix - RESELLERS DISABLED - COMPLETELY NEW FILE
...
export default function Worksheet() {
  ...
  const { data: allOrderItems = [], isLoading: isLoadingOrderItems } = useQuery<OrderItem[]>({
    queryKey: ['/api/items'], // FIXED: Changed from /api/order-items to /api/items
  ...
  const activeOrders = (allOrders || []).filter(order => 
    // !order.archived &&  // TEMPORARILY DISABLED
    order.status !== 'shipping' && 
    order.status !== 'delivered' && 
    order.status !== 'cancelled'
    // order.status !== 'archived'  // TEMPORARILY DISABLED
  );
  ...
  const BYPASS_CACHE_NEW_FILE = "worksheet-new-file-2025-01-27";
  ...
  const updateOrderItemStatusMutation = useMutation({
    ...
    onMutate: async ({itemId, status, checked}) => {
      ...
      // Snapshot the previous value
      const previousItems = queryClient.getQueryData(['/api/items']) as OrderItem[];
      
      // Optimistically update the item
        queryClient.setQueryData(['/api/items'], (old: OrderItem[] | undefined) => {
        if (!old) return old;
        return old.map(item => {
          if (item.id === itemId) {
    ...
    onError: (_, __, context) => {
      ...
      if (context?.previousItems) {
        queryClient.setQueryData(['/api/items'], context.previousItems);
      }
      toast({
        variant: "destructive",
        title: "Update Failed",
  ...
  const improvedFilteredOrderItems = useMemo(() => {
    ...
    Object.keys(itemsByOrderForFiltering).forEach(orderIdStr => {
      ...
      const isShowable = 
          ...
          // parentOrder.archived !== true; // TEMPORARILY DISABLED TO SHOW ORDERS
...
function getNoteTuningFromSpecifications(order: Order | OrderItem): string | undefined {
  ...
  const WorksheetFiltersNew = ({
    ...
    const notStartedItems = useMemo(() => {
      ...
      const validOrderIds = new Set(
        (allOrders || [])
          .filter(order => true) // TEMPORARILY DISABLED ARCHIVED FILTER
          .map(order => order.id)
      );
  ...
  const handleItemStatusChange = (itemId: number, status: string, checked: boolean) => {
    ...
    try {
      ...
      const items = queryClient.getQueryData<OrderItem[]>(['/api/items']) || [];
  ...
  const handleOrderClick = (order: Order) => {
    ...
    const orderItems = queryClient.getQueryData<OrderItem[]>(['/api/items']) || [];
