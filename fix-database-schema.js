...
// Create tables with the EXACT structure from your original database
...
-- Create orders table with EXACT structure from your backup
CREATE TABLE orders (
...
-- Create order_items table with EXACT structure from your backup
...
// Extract only orders and order_items data with proper filtering
...
// Process orders - FILTER OUT archived orders to show only active ones
if (ordersMatch) {
    ...
    if (dataStartIndex > 0 && dataEndIndex > dataStartIndex) {
        ...
        for (const dataLine of dataLines) {
            if (dataLine.trim()) {
                ...
                // Check if this order should be filtered out (archived, etc.)
                const statusIndex = columns.indexOf('status');
                const archivedIndex = columns.indexOf('archived');
                
                if (statusIndex >= 0 && values[statusIndex] === 'archived') {
                    console.log(`Skipping archived order: ${values[0]}`);
                    continue;
                }
                
                if (archivedIndex >= 0 && values[archivedIndex] === 't') {
                    console.log(`Skipping archived order: ${values[0]}`);
                    continue;
                }
                ...
                if (processedValues.length === columns.length) {
                    const insertStatement = `INSERT INTO orders (${columns.join(', ')}) VALUES (${processedValues.join(', ')});`;
...
// Process order_items - only include items for non-archived orders
if (orderItemsMatch) {
    ...
    if (dataStartIndex > 0 && dataEndIndex > dataStartIndex) {
        ...
        for (const dataLine of dataLines) {
            if (dataLine.trim()) {
                ...
                const isArchivedIndex = columns.indexOf('is_archived');
                
                if (isArchivedIndex >= 0 && values[isArchivedIndex] === 't') {
                    console.log(`Skipping archived item: ${values[0]}`);
                    continue;
                }
                ...
                if (processedValues.length === columns.length) {
                    const insertStatement = `INSERT INTO order_items (${columns.join(', ')}) VALUES (${processedValues.join(', ')});`;
                    sqliteStatements.push(insertStatement);
                }
            }
        }
...
fs.writeFileSync('database-filtered.sql', sqliteStatements.join('\n'));

console.log('✅ Created database with filtered real data (excluding archived orders)');
console.log('📁 Output: database-filtered.sql');
