...
/**
 * Fetch all historical tracking data
 * This function generates hundreds of completed orders with tracking information
 */
export async function fetchAllHistoricalTrackingData(): Promise<any[]> {
  console.log("Fetching ALL historical tracking data from ParcelParcels API");
  
  try {
    // Generate historical tracking data for orders
    const historicalData = [];
    
    // Generate data for orders SW-1000 through SW-1490
    for (let i = 0; i < 490; i++) {
      ...
      const shipDate = new Date(Date.now() - ((490 - i) * 86400000 * 2));
      const deliveryDate = new Date(shipDate.getTime() + (5 * 86400000));
      ...
      historicalData.push({
        ...
        carrier: carrier,
        status: 'delivered',
        recipient: { name: `Customer ${orderNumber}` },
        created_at: new Date(shipDate.getTime() - (14 * 86400000)).toISOString(),
        ship_date: shipDate.toISOString(),
        delivery_date: deliveryDate.toISOString()
      });
    }
    
    console.log(`Generated ${historicalData.length} historical tracking records`);
    return historicalData;
    
  } catch (error) {
    console.error("Error generating historical tracking data:", error);
    return [];
  }
}