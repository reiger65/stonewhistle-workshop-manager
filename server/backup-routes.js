...
// API endpoint om een backup te downloaden
router.get('/download/:filename', (req, res) => {
  try {
    ...
    // Stel de headers in voor file download
    ...
    console.error('Fout bij downloaden van backup:', error);
    res.status(500).json({
      success: false,
...
// API endpoint om een backup bestand te verwijderen
router.delete('/delete/:filename', (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: `Backup ${filename} is succesvol verwijderd`
    });
    
  } catch (error) {
    console.error('Fout bij verwijderen van backup:', error);
    res.status(500).json({
      success: false,
...
// API endpoint om een backup bestand te uploaden en te herstellen
router.post('/upload', async (req, res) => {
  try {
    ...
    if (!req.files || !req.files.backup) {
      return res.status(400).json({
        success: false,
        error: 'Geen bestand geüpload of ontbrekend veld "backup"'
      });
    }
    ...
    // Controleer of het een SQL bestand is
    if (!fileName.endsWith('.sql')) {
      return res.status(400).json({
        success: false,
        error: 'Alleen SQL bestanden worden ondersteund'
      });
    }
    
    // Genereer een veilige bestandsnaam met timestamp
    const now = new Date();
    const timestamp = now.toISOString().replace(/:/g, '-').replace(/\./g, '-');
    const safeFileName = `uploaded-backup-${timestamp}.sql`;
    const backupPath = path.join(rootDir, 'backups', safeFileName);
    
    // Verplaats het geüploade bestand naar de backups map
    await new Promise((resolve, reject) => {
      uploadedFile.mv(backupPath, (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
    
    // Stuur succes bericht
    res.status(200).json({
      success: true,
      message: 'Backup bestand succesvol geüpload',
      originalName: fileName,
      savedAs: safeFileName
    });
    
    // Start het restore proces in de achtergrond
    console.log(`Restore gestart voor geüpload bestand: ${safeFileName}`);
    
    // Start het restore proces in de achtergrond
    const restoreScript = path.join(rootDir, 'restore-database.js');
    
    // We geven --restart flag mee om aan te geven dat de applicatie moet herstarten na de restore
    exec(`node ${restoreScript} ${backupPath} --restart`, (error, stdout, stderr) => {
      if (error) {
        console.error(`Restore fout: ${error.message}`);
        return;
      }
    ...
    console.error('Fout bij uploaden van backup bestand:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het uploaden van het backup bestand'
    });
  }
});

// API endpoint om een geüpload backup bestand te verwerken
router.post('/restore', (req, res) => {
  try {
    console.log('Backup restore bestand upload ontvangen');
    
    // Controleer of we een Express-FileUpload request hebben
    if (!req.files || !req.files.backup) {
      return res.status(400).json({
        success: false,
        error: 'Geen backup bestand geüpload'
      });
    }
    ...
    // Valideer bestandstype (moet SQL zijn)
    if (!fileName.toLowerCase().endsWith('.sql')) {
      return res.status(400).json({
        success: false,
        error: 'Ongeldig bestandstype - alleen SQL backup bestanden zijn toegestaan'
      });
    }
    ...
    // Sla het bestand op
    uploadedFile.mv(backupPath, async (err) => {
      if (err) {
        console.error('Fout bij opslaan van geüpload bestand:', err);
        return res.status(500).json({
          success: false,
          error: `Fout bij opslaan van bestand: ${err.message}`
        });
      }
      
      // Bestand is opgeslagen
      console.log(`Backup bestand succesvol geüpload en opgeslagen als: ${safeFileName}`);
    ...
    console.error('Fout bij verwerken van geüpload backup bestand:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het verwerken van het backup bestand'
    });
  }
});
